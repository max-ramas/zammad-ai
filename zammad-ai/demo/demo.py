        ...
        with gr.Column():
            ...
            with gr.Row():
                gr.Button("Terminanfrage", size="sm").click(
                    lambda: (
                        "ISehr geehrten Damen und Herren ich möchte gerne einen Termin für den umtausch meiner Führerschein.Mit freundichen Grüßen.Samira. , Bosnia und Herzegovina Straße und 51 @live.it "
                    ),
                    outputs=input_text,
                )
                gr.Button("Anfrage_Bearbeitungsstand", size="sm").click(
                    lambda: (
                        "Sehr geehrte Damen und Herren, bis wann ist denn mit der Bearbeitung des Umtausches zu rechnen?Ich würde gern im kommenden Urlaub einen gültigen Führerschein bei mir haben :) Danke und freundliche Grüße,Stefan Bär Straße und . 12 @gmx.net Aktenzeichen (falls vorhanden): KVRII/44212250012487"
                    ),
                    outputs=input_text,
                )
            with gr.Row():
