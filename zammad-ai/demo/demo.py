        ...
        with gr.Column():
            ...
            with gr.Row():
                gr.Button("MA_beantwortet", size="sm").click(
                    lambda: "Guten Tag, unter folgendem Link finden Sie alle Informationen rund um den internationalen Führerschein: Dort können Sie auch direkt online einen Antrag stellen. Alternativ können Sie einen Termin vereinbaren und sich den internationalen Führerschein Vorort ausstellen lassen: Mit freundlichen Grüßen Evelyn Wagner",
                    outputs=input_text,
                )
                gr.Button("Fragen", size="sm").click(
                    lambda: "Sehr geehrte Damen und Herren, wie im Anhang zu sehen ist habe ich eine Bestätigung der bestandenen Praxisprüfung erhalten, mit der ich aber anscheinend noch nicht fahren darf. Muss ich für die Prüfungsbescheinigung für begleitetes Fahren ein Termin ausmachen, weil alle anderen haben diese Bescheinigung eigentlich am Tag ihrer Prüfung direkt erhalten. Mit freundlichen Grüssen Straße und 94 .labus@gmail.com Telefonnummer: +4915758266579 ",
                    outputs=input_text,
                )
            with gr.Row():
                gr.Button("Terminanfrage", size="sm").click(
                    lambda: "ISehr geehrten Damen und Herren ich möchte gerne einen Termin für den umtausch meiner Führerschein.Mit freundichen Grüßen.Samira. , Bosnia und Herzegovina Straße und 51 @live.it ",
                    outputs=input_text,
                )
                gr.Button("Anfrage_Bearbeitungsstand", size="sm").click(
                    lambda: "Sehr geehrte Damen und Herren, bis wann ist denn mit der Bearbeitung des Umtausches zu rechnen?Ich würde gern im kommenden Urlaub einen gültigen Führerschein bei mir haben :) Danke und freundliche Grüße,Stefan Bär Straße und . 12 @gmx.net Aktenzeichen (falls vorhanden): KVRII/44212250012487",
                    outputs=input_text,
                )
            with gr.Row():
                gr.Button("Zuordnung nicht möglich", size="sm").click(
                    lambda: 'Import (96).pdf Import (97).pdf Προωθημένο μήνυμα Από: &lt,buergerservice@muenchen.de&gt,Ημερομηνία: Τετ 19 Μαρ 2025 στις 18:56Θέμα: [Unzustellbar] Nachricht zu großΠρος: &lt,kelimollaj25@gmail.com&gt, Hallo Keli Mollaj, Ihre EMail mit dem Betreff "Führerschein " konnte leider nicht an einen oder mehrere Empfänger zugestellt werden. Die Nachricht hatte eine Größe von 14.99 MB, wir akzeptieren jedoch nur EMails mit einer Größe von bis zu 10 MB. Bitte reduzieren Sie die Größe Ihrer Nachricht und versuchen Sie es erneut. Vielen Dank für Ihr Verständnis. Mit freundlichen Grüßen Postmaster von dbszammad.muenchen.de ',
                    outputs=input_text,
                )
                gr.Button("Nachreichung", size="sm").click(
                    lambda: "Hallo Herr Schepke wie telefonisch vereinbart sende ich Ihnen die Bestätigung des Zertifikates zu .Zu dem wollte ich mich nochmals entschuldigen mich so spät gemeldet zu haben da meine aktuelle Lebenssituation nicht auf dem graden weg ist Streitigkeiten im Elternhaus ist mein grösstes problem . Aktuell wohne ich nichtmehr bei meinen eltern sondern übernachte bei meiner Freundin ich habe leider sehr spät zugriff auf meine briefe erhalten und zudem hat es wirklich lange gedauert mit dem termin für das Gutachten.. wir haben uns geeinigt einmal die woche meine post abholen zu dürfen also .. Ich hoffe das ich mein Zertifikat schnellst möglich bekomme und ihnen das direkt zu senden kann. Denn der Führerschein ist lebensnotwendig für mich .. Mit freundlichen Grüßen Anil Battal Straße und .battal@web.de Telefonnummer: +49 17632910562 Aktenzeichen (falls vorhanden): 44111/ 150173561",
                    outputs=input_text,
                )
